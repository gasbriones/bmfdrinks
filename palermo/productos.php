...
if(mysql_num_rows($result) > 0){
	...
	while ($row = mysql_fetch_array($result)) {
		...
		echo '<a class="more" href="listaDeProductos.php?productoCategoria='.$row['categoriaProducto'].'&productoTipo='.$row['tipoId'].'" style="color: #ffffff; background-color: #FC2500;">Ver lista</a></dd>';
		echo '</dl>';
		echo '</div>';
		echo '</li>';
	}
